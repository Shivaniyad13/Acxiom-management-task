...
const UTILS = {
    ...
    getBasePathPrefix: function() {
        ...
        if (currentPath.includes('/maintenance/') || currentPath.includes('/transactions/') || currentPath.includes('/reports/')) {
            return '';
        }
        return '';
    },
    ...
    loadMenuByRole: function() {
        ...
        const currentPath = window.location.pathname;
        const isInSubdirectory = currentPath.includes('/maintenance/') || currentPath.includes('/transactions/') || currentPath.includes('/reports/');
        const basePath = isInSubdirectory ? '../' : '';

        const menus = {
            admin: [
                { text: 'Dashboard', href: basePath + 'dashboard.html' },
                { text: 'Add Membership', href: basePath + 'maintenance/add_membership.html' },
                { text: 'Update Membership', href: basePath + 'maintenance/update_membership.html' },
                { text: 'Add Book', href: basePath + 'maintenance/add_book.html' },
                { text: 'Update Book', href: basePath + 'maintenance/update_book.html' },
                { text: 'User Management', href: basePath + 'maintenance/user_management.html' },
                { text: 'Issue Book', href: basePath + 'transactions/book_issue.html' },
                { text: 'Return Book', href: basePath + 'transactions/return_book.html' },
        ...
        menuList.innerHTML = roleMenus.map(menu => 
